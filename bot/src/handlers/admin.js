...
const { getAdminKeyboard, getMainKeyboard } = require('../keyboards');
...
const handleAdminCallback = async (bot, chatId, data) => {
  ...
  if (data === 'admin_pending') {
    ...
    for (const t of tickets) {
      ...
      buttons.push([
        { text: `✅ Одобрить #${t.id}`, callback_data: `approve_${t.id}` },
        { text: `❌ Отклонить #${t.id}`, callback_data: `reject_${t.id}` },
      ]);
    }
  ...
  if (data.startsWith('approve_')) {
    ...
    if (result) {
      await bot.sendMessage(chatId, `Заявка *#${ticketId}* одобрена.`, {
        parse_mode: 'Markdown',
        ...getAdminKeyboard(),
      });
    } else {
  ...
  if (data.startsWith('reject_')) {
    ...
    if (result) {
      await bot.sendMessage(chatId, `Заявка *#${ticketId}* отклонена.`, {
        parse_mode: 'Markdown',
        ...getAdminKeyboard(),
      });
    } else {
  ...
  if (data.startsWith('detail_')) {
    ...
    if (['OPEN', 'IN_PROGRESS', 'COMPLETED'].includes(ticket.status)) {
      const statusButtons = [];
      if (ticket.status === 'OPEN') statusButtons.push({ text: 'В работу', callback_data: `setstatus_${ticket.id}_IN_PROGRESS` });
      if (ticket.status === 'IN_PROGRESS') statusButtons.push({ text: 'Выполнена', callback_data: `setstatus_${ticket.id}_COMPLETED` });
      if (ticket.status === 'COMPLETED') statusButtons.push({ text: 'Закрыть', callback_data: `setstatus_${ticket.id}_CLOSED` });
      buttons.push(statusButtons);
    }
    if (['OPEN', 'IN_PROGRESS'].includes(ticket.status)) {
      buttons.push([{ text: 'Назначить исполнителя', callback_data: `assign_${ticket.id}` }]);
    }
    buttons.push([{ text: '◀ Назад', callback_data: 'admin_back' }]);
  ...
  if (data.startsWith('setstatus_')) {
    ...
    if (result) {
      await bot.sendMessage(chatId, `Заявка *#${ticketId}* → *${STATUS_LABELS[newStatus] || newStatus}*`, {
        parse_mode: 'Markdown',
        ...getAdminKeyboard(),
      });
    } else {
  ...
  if (data.startsWith('assign_')) {
    ...
    const buttons = admins.map((a) => [
      { text: a.fullName, callback_data: `doassign_${ticketId}_${a.id}` },
    ]);
    buttons.push([{ text: '◀ Назад', callback_data: `detail_${ticketId}` }]);
  ...
  if (data.startsWith('doassign_')) {
    ...
    if (result) {
      await bot.sendMessage(chatId, `Заявка *#${ticketId}* назначена на *${result.assignee?.fullName || '—'}*`, {
        parse_mode: 'Markdown',
        ...getAdminKeyboard(),
      });
    } else {
...
async function showTicketList(bot, chatId, tickets, title) {
  ...
  for (const t of tickets) {
    ...
    buttons.push([{ text: `Подробнее #${t.id}`, callback_data: `detail_${t.id}` }]);
  }
